...
export default class Simple extends PureComponent {
  ...
  loadData = () => {
    ...
    let metrics = raw_data.meta.params.metrics;
    let assets = raw_data.meta.params.assets;
    let times = raw_data.data[0][raw_data.meta.params.metrics[0]].map(d => d[0]);
    let series = raw_data.data.map(d => {
      return {
        ...
        areaStyle: { normal: {} },
        data: {
          supply_volume: d.supply_volume.map(d => parseFloat(d[1])),
          outstanding_debt: d.outstanding_debt.map(d => parseFloat(d[1])),
        }
      }
    });

    let protocols_sorted = _.orderBy(protocols, [protocol => protocol.name.toLowerCase()]);
    let series_sorted = _.orderBy(series, [series => series.name.toLowerCase()]);
    return [protocols_sorted, times, series_sorted, metrics, assets];
  }
  ...
  getOption = () => {
    let [legend, timestamps, series, metrics, assets] = this.loadData();
  ...
  render() {
    this.loadData();
    const metrics = ["supply_volume", "outstanding_debt"].map(metric =>
      <div className="radio">
        <label>
